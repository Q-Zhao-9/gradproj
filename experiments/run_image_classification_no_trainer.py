...
def parse_args():
    ...
    parser.add_argument("--image_size", type=int, default=224, help="Size of the image.")
    parser.add_argument(
        "--max_eval_samples",
        type=int,
        default=None,
    ...
    parser.add_argument('--num_cycles', type=float, default=0.5, help="Number of cycles for cosine_with_restarts scheduler.")
    parser.add_argument(
...
class EarlyStopping:
    ...
    def __call__(self, eval_metric):
        score = eval_metric.get("accuracy", 0.0)
...
def main():
    args = parse_args()
    print('with_tracking:', args.with_tracking)
    ...
    def compute_class_weights(labels, num_classes)->torch.Tensor:
        counts = [0] * num_classes
        for label in labels:
            counts[label] += 1
        total_samples = len(labels)
        class_weights = [total_samples / (num_classes * count) if count > 0 else 0.0 for count in counts]
        return torch.tensor(class_weights, dtype=torch.float)
    
    train_labels = [example[args.label_column_name] for example in dataset["train"]]
    ...
    logger.info(f"Image processor size: {image_processor.size}")
    ...
    train_transforms = Compose(
        [
            ...
            T.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.7, hue=0.15),
            ToTensor(),
            # normalize,
            # Normalize(mean=torch.tensor([0.41,0.29,0.22]), std=torch.tensor([0.29,0.21,0.16])),
            T.RandomErasing(p=0.1, scale=(0.02, 0.1), inplace=True),
        ]
    )
    ...
    lr_scheduler = get_scheduler(
        ...
        scheduler_specific_kwargs={"num_cycles": args.num_cycles,
                                  },
    )
    ...
    if args.resume_from_checkpoint:
        ...
        all_results = json.load(open(os.path.join(checkpoint_path, "all_results.json"), "r"))

        # Extract `epoch_{i}` or `step_{i}`
        training_difference = os.path.splitext(path)[0]
        # Update lr_scheduler and completed_steps
        print('previous lr base lrs: ', lr_scheduler.scheduler.base_lrs)

        if "epoch" in training_difference:
            starting_epoch = int(training_difference.replace("epoch_", "")) + 1
            resume_step = None
            completed_steps = starting_epoch * num_update_steps_per_epoch
        elif "best" in training_difference:
            # best checkpoint
            extra_info_path = os.path.join(checkpoint_path, "extra_info.json")
            if os.path.exists(extra_info_path):
                with open(extra_info_path, "r") as f:
                    extra_info = json.load(f)
                starting_epoch = extra_info.get("epoch", 0) + 1
                completed_steps = starting_epoch * num_update_steps_per_epoch
                best_accuracy = extra_info.get("accuracy", 0.0)
                early_stopping(extra_info)
                print(f'Resumed from Epoch {starting_epoch-1} with accuracy {best_accuracy:.5f}')
            resume_step = None
        else:
        ...
        lr_scheduler.base_lrs = [args.learning_rate for group in optimizer.param_groups]
        lr_scheduler = accelerator.prepare(lr_scheduler)
        for _ in range(completed_steps):
            lr_scheduler.step()
        # Update lr_scheduler and completed_steps
        print('current lr base lrs: ', lr_scheduler.base_lrs)
            
    # update the progress_bar if load from checkpoint
    progress_bar.update(completed_steps)
    
    criterion = torch.nn.CrossEntropyLoss(weight=class_weights.to(accelerator.device))  
    
    print(accelerator.log_with,accelerator.logging_dir)
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        # print('len of active_dataloader:', len(active_dataloader))
        model_dtype = next(model.parameters()).dtype
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                batch["pixel_values"] = batch["pixel_values"].to(model_dtype)
                outputs = model(pixel_values=batch["pixel_values"], labels=None)
                # loss = outputs.loss
                # print(outputs.logits.dtype, batch["labels"].dtype)
                loss = criterion(outputs.logits.to(torch.float32), batch["labels"]) # use class weighted loss
        ...
        progress_bar.set_postfix(**{k: v for k, v in eval_metric.items()})
        eval_metric['epoch'] = epoch
        all_results.append(eval_metric)
        ...
        if args.checkpointing_steps == "epoch":
            ...
            if args.output_dir is not None:
                output_dir = os.path.join(args.output_dir, output_dir)
            accelerator.save_state(output_dir)
        elif args.checkpointing_steps == "best":
            if eval_metric['accuracy'] > best_accuracy:
                ...
                extra_info = {'epoch': epoch, 'lr': lr_scheduler.get_last_lr(), 'accuracy': best_accuracy}
                with open(os.path.join(output_dir, "extra_info.json"), "w") as f:
                    json.dump(extra_info, f)
                # Save the best model separately
                accelerator.wait_for_everyone()
                unwrapped_model = accelerator.unwrap_model(model)
                unwrapped_model.save_pretrained(
                    os.path.join(args.output_dir, "best_full"), 
                    is_main_process=accelerator.is_main_process, save_function=accelerator.save
                )
                image_processor.save_pretrained(os.path.join(args.output_dir, "best_full"))
